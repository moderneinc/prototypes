...
| [`commit-job-redesign.html`](https://moderneinc.github.io/prototypes/commit-job-redesign.html) | Commit **results** page redesign — Morpheus app shell (icon rail + global nav) wrapping a full-width commit-job view: job header, commit-status + PR-status segmented bars with a **Live sync ↔ PR statuses available** phase switch (existence bar → full Open/Merged/Closed/Couldn't-check breakdown), one composed filter model (activity rail + bar segments + PR-status column + search), and the repositories **datagrid in the real Morpheus `rp-table` style** — checkbox (selection enables Approve/Merge/Close with counts), Actions arrow deep-links the PR in its VCS + `⋮` PR-actions menu, Commit status, **PR status**, Org, Repository, Origin, Base branch, Updated. Per-repo drawer (commit + PR → Changelog); ⟳ Refresh runs a status sweep. Morpheus light/dark. | 2026-07-29 | 2026-07-31 |
