...
| [`results-focus-view.html`](https://moderneinc.github.io/prototypes/results-focus-view.html) | Focus modes + **similar results**: full-screen the results tree or the diff; group by similar results (IntelliJ Find-Usages style) — type-of-change clusters → repositories → exemplar usage; repos roll up under VCS orgs; per-repo "N similar" opens the diff with Show / Back-to-search-results navigation | 2026-06-08 | 2026-06-26 |
...
| [`visualizations-tab-results-view.html`](https://moderneinc.github.io/prototypes/visualizations-tab-results-view.html) | Visualizations tab: master-detail picker with Configure panel | 2026-06-02 | 2026-06-04 |
...
| [`activity-page-redesign.html`](https://moderneinc.github.io/prototypes/activity-page-redesign.html) | Activity page redesign — recent-activity feed layout | 2026-06-18 | 2026-06-18 |
...
| [`commit-job-redesign-v2.html`](https://moderneinc.github.io/prototypes/commit-job-redesign-v2.html) | Commit results **v2** — mass-change **progress timeline** for a ~1,000-repo run: **burn-down step chart** (no interpolation between real check points) with lifecycle **stage bands** in flag colours (hover a band for its live count), **flag-style action runs** (Commit → Draft PRs → Approval → Merge → Close, parallel tracks pack automatically) pinned to one shared dated axis; click flags to **multi-select filter** the table — the playhead jumps to that run's moment so statuses match (Commit run → `Commit`, Merge run → `Merged`); scrubbable **playhead with date lollipop**. Models the **VCS as source of truth**: data stops at the last status refresh, a hatched **sync gap** spans the 3 days to wall-clock **Now** at the right edge, a platform-side **Merge · unsynced** action rides above the hatch (*"merged via platform · awaiting VCS sync"*), and refresh is gated behind a confirm for large fleets (**Sync now** at the boundary + last-updated pill). SCM-convention status colours; every status stamped "as of". | 2026-07-29 | 2026-08-05 |
...
| File | What it explores | Added | Last changed |
| --- | --- | --- | --- |
| [`morpheus-design-kit.html`](https://moderneinc.github.io/prototypes/morpheus-design-kit.html) | **⭐ Start here — the Morpheus design kit.** The reference component library for the rebrand: color tokens, type, radius & spacing, icons, buttons, controls — the canonical how-to-use-it page for anyone picking up the Morpheus visual language | 2026-07-11 | 2026-07-30 |
| [`morpheus-token-platform.html`](https://moderneinc.github.io/prototypes/morpheus-token-platform.html) | **Token platform / inspector** — the design tokens pulled live from Figma (12 primitive ramps, 294 semantic colors in Light + Dark, shadows, spacing/radius/height). Browse and inspect with search and a JSON/code view; a Light/Dark switch that re-skins the tool from its own tokens; inheritance (alias graph) and conditional-per-mode relationships; a calculation readout for alpha-derived colors; multi-platform export (JSON / CSS / Tailwind / Swift / Kotlin); plus **Sync** (Figma ↔ code, CI/CD) and **History** (branches, changes, versions, governance) views — the latter with illustrative activity | 2026-07-31 | 2026-07-31 |
...
| [`morpheus.html`](https://moderneinc.github.io/prototypes/morpheus.html) | **Morpheus theme (v3)** — the skin from `ui-explorations.html` baked in as the sole theme (på() × kron: warm cream / ink-navy light mode, near-black desaturated dark mode, strand-spectrum gradient moments), WCAG 2.1 AA by default; adds a **Dev Center** page (`#devcenter`): ownership stat cards, change-campaign waffle-arch charts, and an OWASP top-ten rose chart with a selectable remediation list; a **Changelog** page (`#changelog`): PR/commit feed with filters, type icons, chip metadata, checks/review status and diff bars; a **Repositories** page (`#repositories`): ingested-repo index with search + include-not-ingested toggle, mono changesets and LST availability pills; a **Deploy artifacts** page (`#deploy`): artifact queue with ecosystem glyphs (Npm/Go/Maven), mono versions, Filters/Columns toolbar and Redeploy/Add actions; and a **Builder** page (`#builder`): recipe graph canvas (star clusters, selected-node halo, floating label, tool rail) beside the recipe list panel (run bar, tabs with counts, nested recipe tree, Options footer) | 2026-07-06 | 2026-08-05 |
| [`morpheus-type-scale-map.html`](https://moderneinc.github.io/prototypes/morpheus-type-scale-map.html) | **Type scale consolidation map** — the audit answer as a page: before/after usage histograms (14 sizes → the Morpheus 8, off-scale in red), the full mapping table with selectors and role reasoning, and live specimens of the remaining scale. Links to the side-by-side compare | ?? | ?? | 2026-07-30 | 2026-07-31 |
| [`neo-type-audit.html`](https://moderneinc.github.io/prototypes/neo-type-audit.html) | **Neo type-token consumption audit** — every Font size token traced into moderne-ui: the three token-to-pixels paths (variant / direct import / theme default), per-token code-site counts with verdicts (dead / side door / live / workhorse), the five findings (355 un-tokenized Typographys, caption collision, homeless 12px, dead h1/h2, body already 14px), and a prioritized cleanup list for the Morpheus migration | ?? | ?? | 2026-07-31 | 2026-07-31 |
| [`morpheus-type-compare.html`](https://moderneinc.github.io/prototypes/morpheus-type-compare.html) | **Type scale comparison** — the full `morpheus.html` app with the compare slider repurposed for typography: both sides light mode, left = the old 14-size type (pre-consolidation), right = the 8-step Morpheus scale (PR #262). Old type / Compare / New type switch; best seen on DevCenter stats, Marketplace descriptions, and the Moddy hero | ?? | ?? | 2026-07-06 | 2026-07-31 |
...
| [`moderne-design-kit-preview.html`](https://moderneinc.github.io/prototypes/moderne-design-kit-preview.html) | Moderne Design Kit preview — dual-mode (dark/light) visual language: palette, type, logo, in-situation surfaces, embedded product-overview deck | 2026-06-24 | 2026-08-04 |
...
| File | What it explores | Added | Last changed |
| --- | --- | --- | --- |
| [`moderne-marketing-kit.html`](https://moderneinc.github.io/prototypes/moderne-marketing-kit.html) | **⭐ Start here — the marketing kit.** Gallery of all marketing collateral: the Practical Agentic Coding Playbook (PDF), event speaker-lineup graphics (light + dark PNG), the lower-thirds editor, chromosome generator, and links to every marketing page below. Light/dark, Morpheus tokens | ?? | ?? | 2026-07-28 | 2026-07-28 |
| [`moderne-product-overview-deck.html`](https://moderneinc.github.io/prototypes/moderne-product-overview-deck.html) | Product overview slide deck — 16 slides across the product line (arrow-key / fullscreen navigation) | 2026-06-24 | 2026-06-24 |
...
| [`email-kit/`](https://moderneinc.github.io/prototypes/email-kit/) | Moderne email template kit — light + dark send-ready HTML emails, gallery with iframe previews, hosted image assets so emails render in Gmail/Outlook | 2026-07-01 | 2026-07-01 |
...
| [`moderne-newsletter.html`](https://moderneinc.github.io/prototypes/moderne-newsletter.html) | **Technical bulletin / newsletter** — send-ready monthly bulletin (June 2026 issue): security lead story, LST mass-ingest, migration guide, recipe authoring, MCP and SaaS v2 updates | 2026-06-25 | 2026-06-25 |
| [`mythos-onepager.html`](https://moderneinc.github.io/prototypes/mythos-onepager.html) | **"Mythos finds. Moderne fixes." one-pager** — positioning page pairing Mythos-class scanning with Moderne remediation: scanning-is-solved narrative, Mythos alone vs. with Moderne comparison, objection-handling FAQ | 2026-06-30 | 2026-07-01 |
| [`moderne-vs-openrewrite-onepager.html`](https://moderneinc.github.io/prototypes/moderne-vs-openrewrite-onepager.html) | **Moderne vs. OpenRewrite one-pager** — print-style comparison table: the open-source engine vs. the commercial platform for large multi-repo codebases | 2026-06-30 | 2026-07-01 |
...
| [`commit-job-redesign.html`](https://moderneinc.github.io/prototypes/commit-job-redesign.html) | Redirect stub — forwards to [`commit-job-redesign-v2.html`](https://moderneinc.github.io/prototypes/commit-job-redesign-v2.html) so old links keep working; the original page lives in [`archive/commit-job-redesign.html`](https://moderneinc.github.io/prototypes/archive/commit-job-redesign.html) | 2026-07-29 | 2026-08-05 |
| [`archive/brand-guidelines.html`](https://moderneinc.github.io/prototypes/archive/brand-guidelines.html) | Brand guidelines, including Product Symbols — superseded by [`moderne-brand-guidelines.html`](https://moderneinc.github.io/prototypes/moderne-brand-guidelines.html) | ?? | ?? | 2026-05-01 | 2026-07-28 |
| [`archive/commit-job-redesign.html`](https://moderneinc.github.io/prototypes/archive/commit-job-redesign.html) | Commit results page redesign (first iteration) — superseded by [`commit-job-redesign-v2.html`](https://moderneinc.github.io/prototypes/commit-job-redesign-v2.html) | 2026-07-29 | 2026-08-05 |
