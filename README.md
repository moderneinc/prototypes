...
| [`codegenome-auth/`](https://moderneinc.github.io/prototypes/codegenome-auth/) | **Code Genome Project sign-in & download-token flow.** Morpheus reskin of the gateway auth pages (sign-in, consent, download token, customer token management) rendered in light + dark with an in-page theme toggle. Two-path flow view (social login / customer account) plus a side-by-side [light + dark comparison](https://moderneinc.github.io/prototypes/codegenome-auth/compare.html). Real gateway output; backs [codegenomeproject](https://github.com/moderneinc/codegenomeproject) #14 (token setup) and #32 (login). |
