...
| [`8531-builder-3-scalable-navigation.html`](https://moderneinc.github.io/prototypes/8531-builder-3-scalable-navigation.html) | **Builder — Scalable navigation (Phase 3)** — keeps the Builder usable at thousands of steps. Defaults to a ~2,000-step recipe: groups **collapse by default with total-step counts** (e.g. `Migration group 32 (12)`); the recipe list is **windowed** (only visible rows render) so it stays smooth even fully expanded (~2,000 rows); a sticky **“In:” breadcrumb** tracks the topmost visible row as you scroll. Morpheus light/dark. `?recipe=small` for the small sample. For moderne-ui#8531. |
