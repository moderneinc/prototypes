# Nordic Theme — Token Migration Plan

**Scope:** evolve the existing `@moderneinc/neo-design` → MUI token pipeline in `moderne-ui` so the application can render the Nordic visual language (as prototyped in [`nordic.html`](https://moderneinc.github.io/prototypes/nordic.html)) **without replacing the token architecture**. No code changes are part of this document.

**Sources:** audit of `moderne-ui` (`themes/moderne-theme.ts`, `pages/_app.tsx`, `styles/global.css`, `constants/devcenter.ts`, `components/styled-components/*`, biome lint plugins, `.claude/rules/neo-design-system.md`) and the Nordic token set in `nordic.html` (WCAG 2.1 AA is baked into those values by default).

---

## 1. Executive summary

The app already has the right bones: an external primitive+semantic token package (`neo-design`), a thin MUI adapter (`moderne-theme.ts`), a component library (`neo-styled-components`) that consumes tokens internally, centralized chart scales, a **latent dark palette that has never been activated**, and lint rules that ban raw values. Nordic is therefore a **re-valuing exercise, not a re-architecture**: ~80% of the theme lands by changing token *values* behind the existing semantic names, ~15% needs new semantic tokens (surface/border tiers, link pairing, data-viz category), and ~5% is targeted component-level change (button radius, side-nav elevation).

The five changes users will actually notice, in descending order of blast radius:

1. **Dark mode exists** — Nordic is dual-mode; the app's latent `darkPalette` gets real values and a toggle.
...
3. **Fonts** — Inter → Geist Sans, JetBrains Mono → Geist Mono (two `next/font` lines + Neo `fontFamily` tokens), plus the Nordic "mono for data" idiom.
4. **Warm neutrals** — cool grey-white surfaces → cream/ink in light, near-black elevation ramp in dark.
5. **Charts** — the categorical scale becomes the Nordic "candy" ramp with mandatory deep boundary rings in light mode (that's what keeps SC 1.4.11 passing).

Accessibility is a constraint, not a follow-up: the Nordic values were tuned so **every text pair ≥ 4.5:1 and every control boundary / chart mark ≥ 3:1 in both modes** (links use the Blue AAA pairing at 8.1:1 light / 6.4:1 dark). The migration must not regress those numbers, so the WCAG table in §9 is the release gate.
...
| **Layout** | `constants/general.ts` (`NAVIGATION_RAIL_WIDTH` 98, `GLOBAL_NAV_HEIGHT` 65), zIndex in theme | Widths match the prototype already. Add Nordic's **viewport-fit feed pattern** (card ends at viewport, internal scroll, sticky headers) as a layout recipe, not tokens. zIndex unchanged. |
| **Component tokens** | `diffAddition/diffRemoval/diffHeader` (theme), `pendingStatusTokens` (shim), `MODERNE_CHART_COLOR_SCALE` / `CHRONO_*` (constants), builder WebGL colors | Diff tokens get Nordic values. `pendingStatusTokens` retires into proper Neo `status.{queued, active, canceled}` with Nordic warn/pending values. Chart constants re-point to the new data-viz semantic tokens. Builder WebGL colors mapped by hand (tokens can't reach shaders) but *sourced* from the token sheet. |

**Redundant/obsolete:** the entire `moderneColors` legacy bag (already `@deprecated`), `CHRONO_MIGRATION_GRADIENT_COLORS` (hard-coded), the `ModerneCard` shadow, `body1/body2` deprecated variants, and the prototype's exploratory `--ds-*` OKLCH sheet in `design-system-explorer-src` (superseded by Nordic — should be marked historical so it doesn't become a fourth naming system).
...
|---|---|---|---|
| `palette.background.default` (`#F8F9FA` / `#1E1E1E`) | → `#F2F0EA` / `#141419` via `semanticColors.surfaces.canvasChrome` | **Update value + new semantic** | Nordic separates chrome canvas (rail/topbar) from content canvas |
...
| — (gap) | `surfaces.sunken` = `#EDEBE2` / `#2A2A32`; `surfaces.sunken2` = `#E4E1D6` / `#33333D` | **Create new** | Nordic `--surface`/`--surface2` (seg controls, code chips, kbd) |
...
| — (gap) | `semanticColors.link.{default, hover}` = `#1245AE` / `#7FA9F9` | **Create new** | Nordic splits links (Blue AAA pairing, 8.1:1 / 6.4:1) from the primary action color; today links ride `primary.main` |
...
| Current token | New token (or new value) | Action | Reason |
|---|---|---|---|
| `semanticColors.buttons.primary.default` | `#232342` bg / `#FBFAF6` fg — dark mode: `#F1F0EA` bg / `#17171C` fg | **Update value** | Ink primary button is the Nordic signature (14.5:1 / 15.6:1 labels) |
...
| `status.warning.*` | text `#935B00` / `#FFA83D` + `rgba(255,168,61,.16/.12)` tints | **Update value** | Nordic amber (Skipped / No LST pills) |
| `pendingStatusTokens` (app shim: `queued/active/canceled` purples) | Neo `status.{queued, active}` = periwinkle pair `#3E5BE8`/`#8CA0F5`; `status.neutral` = muted+sunken pill | **Replace + deprecate shim** | Closes the documented gap (moderne-ui#8435) with Nordic hues instead of the temp purples |
...
| `shadows.card` | `none` (hairline border instead) | **Update value** | Flat Nordic surfaces |
...
| Login / unauthenticated layout | **Low** | Two hard-coded hexes replaced; Nordic login is a new composition |
...
- **Do not add a fourth naming system.** Nordic lands inside Neo's existing `semanticColors.*` dotted-path convention. The prototype's `--nordic`/`--ds-*` CSS names die with the prototype.
...
- [ ] Reference render parity spot-check against `nordic.html` (Dev Center, Repositories table, Moddy, login) in both modes.

---

*Prepared as the productionization companion to the Nordic v3 prototype. No code was modified.*
