      ...
      for (var pi = 0; pi < figma.root.children.length; pi++) {
        ...
        for (var ci = 0; ci < pg.children.length; ci++) {
          ...
          if (k && k.indexOf("mirror/") === 0) {
            var status = node.getPluginData("construct.mirror.status") || "proposed";
            mirrorItems.push({ key: k, slug: k.replace("mirror/", ""), status: status });
          }
        }
      }
  ...
  if (msg.type === "reject-mirror") {
    try {
      ...
      if (rejectNode) {
        ...
        rejectNode.name = "mirror / " + msg.slug + " ✗ REJECTED";
