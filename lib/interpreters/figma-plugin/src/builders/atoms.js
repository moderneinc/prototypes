...
async function syncTextStyles(seenKeys) {
  ...
  for (const spec of TEXT_STYLE_SPECS) {
    ...
    const tagKey = "atom/text-style/" + spec.key;
    seenKeys.add(tagKey);
...
async function syncAtoms(index, seenKeys) {
  const page = await getOrCreatePage(PAGE_NAMES.atom);
  await loadMonoFont();
  let xCursor = 0;
  let yCursor = 0;
  ...
  for (const glyphName of Object.keys(CANONICAL.glyph)) {
    ...
    const setKey = "atom/glyph/" + glyphName;
    const setName = "Construct / Atoms / Glyph / " + glyphName;
    seenKeys.add(setKey);
    ...
    if (!setNode) {
      ...
      try {
        ...
        log("  ! combineAsVariants failed for " + glyphName + ": " + e.message, "err");
      }
    } else {
      // Existing set — make sure children are positioned, leave layout alone.
      log("  · set: " + setName, "dim");
      // Append any newly-created variants into the set
      for (const v of variantNodes) {
        if (v.parent !== setNode) {
          try { setNode.appendChild(v); } catch (e) {}
        }
      }
    }
  }
