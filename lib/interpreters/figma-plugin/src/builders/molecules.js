...
const MOLECULE_SPECS = [
  {
    key: "molecule/section-header-row",
...
function countExpectedMolecules() {
  return MOLECULE_SPECS.length;
}

async function syncMolecules(index, seenKeys) {
  const page = await getOrCreatePage(PAGE_NAMES.molecule);
  ...
  for (const spec of MOLECULE_SPECS) {
    ...
    const fullName = "Construct / Molecules / " + spec.name;
    ...
    log("  " + (isNew ? "+" : "·") + " molecule: " + spec.name, isNew ? "ok" : "dim");
  }
}
