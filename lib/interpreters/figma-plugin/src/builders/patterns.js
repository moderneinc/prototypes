...
async function syncPatterns(index, seenKeys) {
  ...
  for (let ei = 0; ei < page.children.length; ei++) {
    const existing = page.children[ei];
    let col = 0;
    for (let ci = 1; ci < COLS; ci++) {
      if (Math.abs(existing.x - ci * (PATTERN_W + COL_GAP)) < Math.abs(existing.x - col * (PATTERN_W + COL_GAP))) {
        col = ci;
      }
    }
    const bottom = existing.y + existing.height + ROW_GAP;
  ...
  // Reflow: re-stack all items in each column with proper gaps.
  // This prevents overlap when existing items grow taller after content changes.
  const reflowCols = [[], [], []];
  for (var ri = 0; ri < page.children.length; ri++) {
    var child = page.children[ri];
    var rcol = Math.round(child.x / (PATTERN_W + COL_GAP));
    if (rcol >= 0 && rcol < COLS) reflowCols[rcol].push(child);
  }
