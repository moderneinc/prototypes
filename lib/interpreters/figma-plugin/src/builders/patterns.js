...
// All patterns carry construct.review="after-first-designer-use" — open
// question whether these belong in the plugin or are downstream of it.

const PATTERN_SPECS = [
  {
    key: "pattern/help-screen-top-level",
  ...
  {
    key: "pattern/error-screen-compact",
...
async function syncPatterns(index, seenKeys) {
  ...
  for (const spec of PATTERN_SPECS) {
    ...
    const fullName = "Construct / Patterns / " + spec.name;
    ...
    if (isNew) {
      xCursor += PATTERN_W + GAP;
      if (xCursor > 2400) { xCursor = 0; yCursor += 800; }
    }

    log("  " + (isNew ? "+" : "·") + " pattern: " + spec.name + "  (review: after-first-designer-use)", isNew ? "ok" : "dim");
  }
}
