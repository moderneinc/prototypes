...
function computeCanonicalHashes(canonical, patterns) {
  ...
  // Resolved token values — catches token-level changes (e.g. color hex update)
  // that don't change the structural references but do change what renders.
  hashes["token/colors"] = stableHash(canonical.color);
  hashes["token/typography"] = stableHash(canonical.typography);
  hashes["token/spacing"] = stableHash(canonical.spacing);
  hashes["token/banners"] = stableHash(canonical.banner);
